...
判据落在**接口面**：引入新的配置项、接口、页面或数据模型算 MINOR；在既有能力内修正行为、补上缺口算 PATCH。像调整告警的重复间隔、补充消息字段、给任务恢复补一条通知，都属于后者。

版本状态：

1. 开发期使用 `X.Y.Z-SNAPSHOT`，可自由提交，版本号不承诺稳定。
2. 定版时去掉 `-SNAPSHOT`，提交后打标签 `vX.Y.Z`，再从该提交或标签构建发布包。
3. 发布包只从定版提交构建，不从后续 `-SNAPSHOT` 构建。**下一版的 `-SNAPSHOT` 在开始新一轮改动时切换**，作为那一轮工作的第一个提交：这样 pom 停留在已发布版本的时间窗口，正好对应 `target/` 中保留待部署包的窗口。不要在定版后立刻切换——构建会清空 `target/`（发布脚本要求其中恰好只有一套产物），会覆盖掉尚未部署的包。
