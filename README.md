...
开发期使用 `X.Y.Z-SNAPSHOT`。定版时去掉后缀、同步把 CHANGELOG 的 `Unreleased` 段落改为 `## X.Y.Z - YYYY-MM-DD`，提交后打 `vX.Y.Z` 标签。发布包只从定版提交构建，不从后续 `-SNAPSHOT` 构建；已打标签的版本冻结，需要修复时递增补丁位发布新版本。递增规则由维护者掌握。

页面显示的版本来自 JAR manifest 的 `Implementation-Version`，只有重新构建发布后才会变化。功能和修复使用范围清晰的 Conventional Commit；Git 操作遵循当前用户授权及适用的 AGENTS.md，不将历史计划视为自动提交或推送的授权。
